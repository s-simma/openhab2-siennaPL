...
public class Bits {
    ...
    public static void setBitsOfBytes(long value, byte[] data, int startByte, int startBit, int endByte, int endBit) {
        ...
        if (startByte == endByte) {
            value1 = setBitsOfBytesUtil(value1, data, startByte, startBit, endBit);
        } else {
            for (int i = endByte; i >= startByte; --i) {
                ...
                value1 = setBitsOfBytesUtil(value1, data, i, localStartBit, localEndBit);
            }
        }

        if (value1 != 0) {
            throw new IllegalArgumentException(
                    String.format("The value does not fit in the bit range (remaining: %d)", value1));
        }
    }
